...
if($_SERVER['REQUEST_METHOD'] == "POST"){
  ...
  if( ! isset($_GET['do'])){
  $sql = "INSERT INTO Deathreports(patient_name,date_of_death,title,description,doctor) VALUES ('$patient_name','$date_of_death','$title','$description','$doctor')";
  $con ->exec($sql);
  header('Location: DeathReports.php?do=" " ');
      ...
      else if ($_GET['do'] == 'Edit')
    {
           ...
           $id = $_GET['id'];
       $stmt = $con ->prepare("UPDATE Deathreports SET patient_name = ? ,date_of_death = ?,title = ?,description = ?,doctor = ? WHERE id = ? ");
       $stmt ->execute(array($patient_name,$date_of_death,$title,$description,$doctor,$id));
       header('Location: DeathReports.php?do=" " ');
...
<body>
   ...
   <?php include 'navbar/navbar.php'; ?> 
    ...
    <div class="panel-body panel-form">
             ...
             <form action="addDeathreport.php" method="POST" class="form-horizontal" >
     <div class="form-group row">
              ...
              <label  class="control-label col-sm-2">Date <i class="text-danger">*</i></label>
                <div class="col-sm-10">
                <input type="date" name="date_of_birth" placeholder="date"  class="datepicker form-control" required="" >
               </div>
            </div> 
