 ...
 <?php  
    ...
    global $i;
       $i = 1 ;
    include 'contact.php' ;
   $sql = $con->prepare("SELECT * FROM employees WHERE Role = 'Case_manager' ");
        $sql->execute();
    ...
    <div class="panel-body"> 
        <div  class="table-responsive ">
               <table class="datatable table table-striped table-bordered"  cellspacing="0" width="100%">
                 <h1 class="btn btn-success btn-block btn-lg" >Case manager List </h1>
                <thead>
                        <tr>
                            <th>NO</th>
                            <th>Picture</th>
   ...
   <?php    
        ...
        ?>
         ...
         <tr>
             ...
             <td><a class="btn btn-primary" href="CaseManagerList.php?do=Edit&id=<?php echo $row['id'] ?>"> edit</a>
                 <a class="btn btn-danger" href="CaseManagerList.php?do=delete&id=<?php echo $row['id'] ?>"> delete</a>
                 <a class="btn btn-info"   href="profile.php?id=<?php echo $row['id'] ?>&user=Employee&Role=Case_manager"> show</a> </td>  
        </tr>
        <?php } ?></tbody>
      </table>
       </div>
     ...
     <?php  } else if($do == 'Edit'){
        ...
        if($_SESSION['Role'] != "Admin"){
           ...
           $id = $_GET['id'];
           $sql = $con->prepare("SELECT * FROM employees Where id = ? AND Role = 'Case_manager' ");
                ...
                <div class="col-sm-10">
                    <select name="Role" class="form-control" >
                      <option value="" selected="selected">Select Option</option>
                        ...
                        <option selected="selected">Case_manager</option>
                    </select>
                     </div>
            </div> 
                      <div class="form-group row">
            ...
            <div class="form-group row">
              ...
              exit();
                            ...
                            $sql = $con->prepare("SELECT * FROM employees Where id = ? AND Role = 'Case_manager' ");
                             ...
                             $stmt = $con ->prepare(" DELETE FROM employees Where id = ? AND Role = 'Case_manager' ");
